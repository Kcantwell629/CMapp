...
$page_title = 'Delete Descriptions';
include ('includes/header.html');

print '<h2>Delete This Description</h2>';
...
if (isset($_GET['desc_id']) && is_numeric($_GET['desc_id']) && ($_GET['desc_id'] > 0)) {
	...
	if ($r = mysqli_query($dbc, $query)) { // Retrieve the information.
		...
		// Make the form:
		print '<form action="delete_customer.php" method="post">
		...
		<p><input type="submit" name="submit" value="Delete This Description!"></p>
		</form>';
	...
	// Define the query:
	$query = "DELETE FROM descriptions WHERE description_id={$_POST['desc_id']} LIMIT 1";

	if ($result = mysqli_query($dbc, $query)){
		print '<h3>The description has been deleted!</h3>';
	} else {
