...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if ($uid) { // If everything's OK.
		...
		if (mysqli_affected_rows($dbc) == 1) { // If it ran OK.
			...
			// Send an email:
			$body = "Your password to log into CMapp has been temporarily 
			changed to '$p'. Please log in using this password and this email address.
			Then you may change your password to something more familiar.";
			mail ($_POST['email'], 'Your temporary password.', $body, 'From: kcantwell629@gmail.com');
			
			// Print a message and wrap up:
			echo '<h3>Your password has been changed. You will receive the new, temporary password at the
			email address with which you registered. Once you have logged in with this password, you may change
			it by clicking on the "Change Password" link.</h3>';
			mysqli_close($dbc);
